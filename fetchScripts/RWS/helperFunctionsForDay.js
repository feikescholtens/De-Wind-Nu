import { add, addDays, addHours, isSameDay, sub, subDays } from "date-fns";
...
function getRWSFetchDates(dateParsed, DSTDates) {
	...
	if (dateParsed > DSTDates.toDST && dateParsed < DSTDates.fromDST) {
		...
		if (isSameDay(dateStartFetch, DSTDates.toDST) && dateStartFetch.getHours() === 22)
			dateStartFetch = addHours(dateStartFetch, 1);
		...
		startTime = "22:00:00";
		endTime = "00:00:00";
		dateStartFetch = formatInTimeZone(
			sub(dateParsed, { days: 1 }),
			global.userTimeZone,
			"yyyy-MM-dd",
		);
		dateEndFetch = formatInTimeZone(
			add(dateParsed, { days: 1, hours: 2 }),
			global.userTimeZone,
			"yyyy-MM-dd",
		);
	} else {
