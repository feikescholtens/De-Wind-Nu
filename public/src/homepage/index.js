...
import {
	...
	changeOverviewForm,
	changeTiles,
	distanceLocationToCurrentLocation,
	getLocationToUse,
	showLocationPreferenceOptions,
} from "./js/index_HelperFunctions.js";
import { initList, initMap } from "./js/mapOrListInit.js";
import { panMapToLocation, setOverviewListData, setOverviewMapData } from "./js/sharedFunctions.js";
...
document
	...
	.addEventListener("click", (e) => changeOverviewForm(overviewFormSelector, e)); //in the select bar
document
...
acquireLocation().then((locationToUse) => {
	...
	if (locationToUse) {
		for (const id in data)
			data[id].distance = distanceLocationToCurrentLocation(
				locationToUse.lat,
				locationToUse.lon,
				data[id].lat,
				data[id].lon,
...
document.querySelectorAll("[data-mapfit]").forEach((element) => {
	element.addEventListener("click", () => {
		if (localStorage.getItem("overviewForm") === "map") {
			if (localStorage.getItem("locationPreference") === "none") {
				fitMapToMarkers(map, markersLats, markersLons);
				return;
			}

			const locationToUse = getLocationToUse();
			if (locationToUse && !map.getBounds().contains([locationToUse.lon, locationToUse.lat])) {
