...
export async function fetchDataForOverview_KNMI(locations, resolve) {
  ...
  const rawDataString = await fetch(`https://api.dataplatform.knmi.nl/edr/v1/collections/10-minute-in-situ-meteorological-observations/cube?bbox=-180,-90,180,90&datetime=${beginDate}/..&parameter-name=ff,gff,dd`, { headers: { "Authorization": process.env.KDP_EDR_KEY } })
