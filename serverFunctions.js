...
export async function setEnvironmentVariables() {
	...
	if (global.port === 3000) {
		...
		[
			...
			promises.push(setEnvironmentVariableFromGoogleSecretManager(identifier));
		});
	}
}
...
export function getActiveLocations() {
	...
	const activeLocations = Object.entries(locations).filter(([, value]) => value.active);
...
export function initializeExpress() {
	...
	app.use(
		"/robots.txt",
		cors(),
		express.static(path.resolve(__dirname, "public/assets/robots.txt")),
	);
...
export function getLocationListParsingHarmonie(response, locations) {
	const locationsGCP = [];
...
export async function getClientIPLocation(request, response) {
	...
	const IPQualityScore = await fetch(
		`https://ipqualityscore.com/api/json/ip/${process.env.IPQUALITYSCORE_KEY}/${clientIP}?strictness=0&allow_public_access_points=true&fast=true&lighter_penalties=true&mobile=true`,
	)
		.catch((error) => {
			console.log(error);
	...
	if (IPQualityScore.fraud_score === 0)
		response.json({
			success: true,
			lowEnoughIPScore: true,
			lat: IPQualityScore.latitude,
			lon: IPQualityScore.longitude,
		});
