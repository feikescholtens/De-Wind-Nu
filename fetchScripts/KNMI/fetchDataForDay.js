...
import {
	ISO_StringToLocalHHmm,
	processAllNegativeArrays,
	theoreticalMeasurements,
} from "../fetchUtilFunctions.js";
...
export async function fetchDataForDay_KNMI(dateParsed, databaseData, resolve, times, DSTDates) {
	...
	const rawDataString = await fetch(
		`https://api.dataplatform.knmi.nl/edr/v1/collections/10-minute-in-situ-meteorological-observations/locations/${KNMI_ID}?datetime=${dateStartFetch}/${dateEndFetch}&parameter-name=ff,gff,dd`,
		{ headers: { Authorization: process.env.KDP_EDR_KEY } },
	)
		.then((response) => response.text())
		.catch((error) => catchFetchError(resolve, data, error, "KNMI")); // This handles all errors that can occur during the fetch, like timeouts or no internet connection
	try {
	...
	rawData.coverages[0].domain.axes.t.values.forEach((measurementTime) => {
		measurementTimes.push(ISO_StringToLocalHHmm(measurementTime, measurementTimes));
	...
	times.forEach((timeStamp) => {
		...
		if (rawData.coverages[0].ranges?.ff?.values[indexTime] != undefined)
			windSpeed.push(rawData.coverages[0].ranges.ff.values[indexTime] * 1.94384449); // Convert m/s to knots
		else windSpeed.push(-999);

		// biome-ignore lint/suspicious/noDoubleEquals: Needed to check for both null and undefined
		if (rawData.coverages[0].ranges?.gff?.values[indexTime] != undefined)
			windGust.push(rawData.coverages[0].ranges.gff.values[indexTime] * 1.94384449); // Convert m/s to knots
		else windGust.push(-999);

		// biome-ignore lint/suspicious/noDoubleEquals: Needed to check for both null and undefined
		if (rawData.coverages[0].ranges?.dd?.values[indexTime] != undefined)
			windDirection.push(rawData.coverages[0].ranges.dd.values[indexTime]);
