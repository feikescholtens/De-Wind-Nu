...
import { theoreticalMeasurements, ISO_StringToLocalHHmm } from "../fetchUtilFunctions.js"
...
export async function fetchDataForDay_MVB(dateParsed, databaseData, resolve, times, DSTDates) {
  ...
  const rawDataString = await fetch("https://api.meetnetvlaamsebanken.be/V2/getData", await giveMVBFetchOptions(dateParsed, DSTDates, databaseData, resolve))
    .then(response => response.text()).catch((error) => catchFetchError(resolve, data, error, "MVB")) // This handles all errors that can occur during the fetch, like timeouts or no internet connection
  ...
  rawData.Values.forEach(parameter => { // Loop through all the parameters in the data, like wind speed, gusts and direction
    ...
    parameter.Values.forEach(measurement => measurementTimes.push(ISO_StringToLocalHHmm(measurement.Timestamp, measurementTimes))) // Add all the times of the measurements to the measurementTimes array
    ...
    times.forEach(timeStamp => { // Loop through all the times that are requested [00:00, 00:10, ..., 00:00_nextDay]
      ...
      if (parameter.Values[indexTime] == undefined) { tempArray.push(-999); return }
      if (parameter.Values[indexTime].Value == undefined) { tempArray.push(-999); return }
    ...
    if (parameter.ID.includes("WVC")) windSpeed = tempArray.customCopy().map(x => x * 1.94384449) // Convert m/s to knots
    if (parameter.ID.includes("WC3")) windGusts = tempArray.customCopy().map(x => x * 1.94384449) // Convert m/s to knots
    if (parameter.ID.includes("WRS")) windDirection = tempArray.customCopy()
  })
