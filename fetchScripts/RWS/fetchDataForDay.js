...
export async function fetchDataForDay_RWS(dateParsed, databaseData, resolve, times, DSTDates) {
	...
	const rawDataString = await fetch(
		...
		.catch((error) => catchFetchError(resolve, data, error, "RWS")); // This handles all errors that can occur during the fetch, like timeouts or no internet connection
	try {
	...
	if (rawData.WaarnemingenLijst == undefined) {
		...
		return;
	} // If there is no data, return empty arrays. This should not happen, since SuccesvolFalseError should have already returned
