...
if (!buildPage || buildPage === "homepage") {
	configs.push(
		buildWebPageBundleConfig(homepageJS, homepageHTML, homepageDestinationDir, homepagePathURL),
	);
}
...
function minifyJS(inputFile, outputFile, pathURL) {
	return {
		async closeBundle() {
			//Hook when bundling is complete

			const uglifyResult = UglifyJS.minify(
				{ minified: readFileSync(inputFile).toString() },
				(() => {
					if (makeSourceMaps) {
						return {
							sourceMap: {
