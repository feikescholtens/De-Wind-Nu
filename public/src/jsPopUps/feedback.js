	...
	if (!document.getElementsByClassName("messageBox")[0]) {
		...
		document.querySelector("[data-send]").addEventListener("click", async () => {
			...
			if (message === "") emptyMessage(messageContainer);
				...
				document
					.getElementsByClassName("markerContainer center noDisplay")[0]
				...
				const data = await fetch("/addFeedback", options).then((response) => response.text());
				...
				if (data === "250 2.0.0 OK") {
					...
					responseMessage.innerHTML =
						"Verzenden mislukt. Contacteer <a href='mailto:dewindnu@gmail.com'>dewindnu@gmail.com</a>";
