...
import { addDays, format, isToday, parse, parseISO, startOfDay } from "date-fns";
...
import {
	...
	switchNextDay,
	switchPreviousDay,
} from "./js/index_HelperFunctions.js";

redirect();
updateLocalVariables();
handleTimeZoneWarning();
if (isIOS())
	document.getElementById("settings").style.width = `${document.body.clientWidth - 40}px`;
...
if (dateURL !== null)
	document.querySelector("[data-datePicker]").value = format(dateURL, "yyyy-MM-dd");
fetchData(dateISO);
...
function fetchData(date) {
	...
	fetch(`/getData/${locationID}?date=${date}`).then(async (response) => {
		clearInterval(changeNumbersLoadingSymbolInterval);
		hideLoader();

		if (response.status !== 200) {
			document.querySelector("[data-errorFetching]").innerText =
				`Data ophalen van server mislukt met status code ${response.status}!`;
			showErrorMessage();
...
if (localStorage.getItem("dataForm") === "table") {
	//This is done here (instead of in the index.ejs of the homepage) since it isn't displayed after all the data is processed
...
function applyCanvasDynamicStyles() {
	...
	currentWindBox.style.width =
		currentWindBox.style.height = `${(compassCanvas.clientWidth * (212 / currentWindBoxSize)) / Math.sqrt(2)}px`;
...
[document.querySelector("[data-previousDay]"), document.querySelector("[data-nextDay]")].forEach(
	(arrow) => {
...
document
	.querySelector("[data-currentDay]")
	.addEventListener("click", () => document.querySelector("[data-datePicker]").showPicker());
...
document
	.querySelector("[data-graphs]")
	.addEventListener("click", (e) => changeDataForm(dataFormSelector, e)); //in the select bar
document
...
document.querySelectorAll("[data-gobackhome]").forEach((element) => {
	element.addEventListener("click", () => {
		//Somehow the camelcased "data-goBackHome" won't work on Safari
		if (document.referrer) {
			...
			if (
				[
					...
					"www.dewindnu.nl",
					"192.168.2.6",
					"de-wind-nu-test.ew.r.appspot.com",
...
async function processRetrievedData(dataFetched) {
	...
	globalThis.interpolatedIndices = {};
	...
	else if (dataFetched.datasetMeasurements === "MVB")
		measurementSourceLabelNode.innerText = "Metingen: Meetnet Vlaamse Banken";
	else if (dataFetched.datasetMeasurements === "VLINDER")
		measurementSourceLabelNode.innerText = "Metingen: UGent VLINDER project";
	else if (dataFetched.datasetMeasurements === "RWS")
		measurementSourceLabelNode.innerText = "Metingen: Rijkswaterstaat";
	...
	document
		.querySelector("[data-datePicker]")
		.setAttribute("max", format(globalThis.datePickerMax, "yyyy-MM-dd"));
	...
	if (dataWUnits.windGusts.length !== 0 || dataWUnits.windSpeedForecast)
		headingChartWindspeed.innerText = "Windsterkte en -vlagen";
	if (NoMeasurementTypesAvailable === 3 || dataWUnits.windSpeedForecast)
		headingTable.innerText = "Windsterkte, -vlagen en -richting";
	else if (NoMeasurementTypesAvailable !== 3 && !dataWUnits.windSpeedForecast)
		headingTable.innerText = "Windsterkte, en -richting";
