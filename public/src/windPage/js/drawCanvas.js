...
function calcDecimalPlaces(angle) {
	...
	// biome-ignore lint/suspicious/noDoubleEquals: Allowed here
...
export function drawCanvasBackground(ctx) {
	...
	const colourGradientOuterPrimary = getComputedStyle(document.body).getPropertyValue(
		"--lineColour2",
	);
	const colourGradientOuterSecondary = getComputedStyle(document.body).getPropertyValue(
		"--lineColour3",
	);
	const colourGradientOuterTertiary = getComputedStyle(document.body).getPropertyValue(
		"--lineColour1",
	);
	ctx.lineWidth = 2;
	...
	for (let i = 0; i < NoLines; i++) {
		...
		const x_outer =
			(sizeBoxToDrawIn / 2) * Math.cos((Math.PI / 180) * (angle - 90)) +
			0.5 * sizeBoxToDrawIn +
			correctionMiddle;
		const y_outer =
			(sizeBoxToDrawIn / 2) * Math.sin((Math.PI / 180) * (angle - 90)) +
			0.5 * sizeBoxToDrawIn +
			correctionMiddle;
		const x_inner =
			(sizeBoxToDrawIn - 250 + 7) * Math.cos((Math.PI / 180) * (angle - 90)) +
			0.5 * sizeBoxToDrawIn +
			correctionMiddle;
		const y_inner =
			(sizeBoxToDrawIn - 250 + 7) * Math.sin((Math.PI / 180) * (angle - 90)) +
			0.5 * sizeBoxToDrawIn +
			correctionMiddle;
		...
		if (calcDecimalPlaces(angle) === 0 || calcDecimalPlaces(angle) === 1) {
			//e.g. 0, 22.5, 45...
