...
import { theoreticalMeasurements, processAllNegativeArrays } from "../fetchUtilFunctions.js"
...
export async function fetchDataForDay_VLINDER(dateParsed, databaseData, resolve, times, DSTDates) {
  ...
  const rawDataString = await fetch(`https://mooncake.ugent.be/api/measurements/${VLINDER_ID}?start=${dateStartFetch}&end=${dateEndFetch}`)
    .then(response => response.text()).catch((error) => catchFetchError(resolve, data, error, "VLINDER")) // This handles all errors that can occur during the fetch, like timeouts or no internet connection
  ...
  rawData.forEach(measurement => measurementTimes.push(VLINDER_dateStringToLocalHHmm(measurement.time, measurementTimes))) // Add all the times of the measurements to the measurementTimes array
  ...
  times.forEach(timeStamp => { // Loop through all the times that are requested [00:00, 00:10, ..., 00:00_nextDay]
    ...
    if (rawData[indexTime].windSpeed != undefined) windSpeed.push(rawData[indexTime].windSpeed * 0.53995726994149) // Convert km/h to knots
    else windSpeed.push(-999)

    if (rawData[indexTime].windGust != undefined) windGust.push(rawData[indexTime].windGust * 0.53995726994149) // Convert km/h to knots
    else windGust.push(-999)

    if (rawData[indexTime].windDirection != undefined) windDirection.push(rawData[indexTime].windDirection)
