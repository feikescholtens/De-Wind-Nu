...
export function generateTimes(measurementEveryXMinutes, timeZoneChange) {
	...
	for (let i = 0; i < 24; i++) {
		for (let j = 0; j < 60 / measurementEveryXMinutes; j++) {
			const hour = i;
			const minute = measurementEveryXMinutes * j;
	...
	if (timeZoneChange) {
		...
		if (timeZoneChange === "fromDST") {
			...
			const middle = times.slice(
				2 * (60 / measurementEveryXMinutes),
				3 * (60 / measurementEveryXMinutes),
			);
			const end = times.slice(3 * (60 / measurementEveryXMinutes));
...
export function calcInterpolation(array, times, startTimeIndexInTimes, stopTimeIndexInTimes) {
	...
	for (let i = startTimeIndexInTimes; i < array.length; i++) {
		...
		// biome-ignore lint/suspicious/noDoubleEquals: Allows checking for undefined or null
		if (array[i] == undefined) {
			...
			for (let k = startIndex; k < stopIndex - 1; k++) {
				const value =
					startValue + ((stopValue - startValue) / (stopIndex - startIndex)) * (k + 1 - startIndex);
				if (value !== undefined)
					interpolatedData.push({ time: times[k + 1], index: k + 1, value: value });
			}
			i = j;
		}
	}
