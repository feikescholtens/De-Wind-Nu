...
import { add, isBefore, isFuture, isToday, isValid, parseISO } from "date-fns";
...
import {
	...
	getTimeChangeDates,
	startOfDayTimeZone,
} from "./getScriptUtilFunctions.js";
import { isExistingLocation, log } from "./serverFunctions.js";
...
export async function getData(request, response, date, locations) {
	...
	values.times = structuredClone(times);
	times[times.length - 1] = "00:00_nextDay";

	//Measurements
	const dataFetched = await new Promise((resolve) => {
		if (datasetMeasurements == null) {
			resolve(null);
			return;
		} //This will run if the location is forecast only
		if (isFuture(dateParsed)) {
			resolve(null);
			return;
		}

		if (datasetMeasurements === "VLINDER")
			return fetchDataForDay_VLINDER(dateParsed, location, resolve, times, DSTDates);
		if (datasetMeasurements === "RWS")
			return fetchDataForDay_RWS(dateParsed, location, resolve, times, DSTDates);
		if (datasetMeasurements === "KNMI")
			return fetchDataForDay_KNMI(dateParsed, location, resolve, times, DSTDates);
		if (datasetMeasurements === "MVB")
			return fetchDataForDay_MVB(dateParsed, location, resolve, times, DSTDates);
	});
	...
	//Forecast
	let forecastObj,
		forecastInfoString = "niet beschikbaar";
	...
	if (!isBefore(dateParsed, startOfDayTimeZone(new Date(), timeZone))) {
		...
		const forecastData =
			(await (await firestore.doc("Harmonie forecast today & future v2/document").get()).data()) ||
			{};
	...
	if (forecastObj) {
		...
		let stopIndex;
		const tempIndex = forecastObj
			.slice(startIndex)
			.findIndex((location) => location.date !== dateFormatted);
		...
		if (startIndex !== -1 && startIndex !== forecastObj.length - 1) {
			...
			const windSpeedForecast = new Array(times.length),
			...
			for (let i = 0; i < amountHourValues; i++) {
				windSpeedForecast[startTimeIndexInTimes + i * NoMeasurementsXHour] =
					forecastObj[i + startIndex].s;
				windGustsForecast[startTimeIndexInTimes + i * NoMeasurementsXHour] =
					forecastObj[i + startIndex].g;
				windDirectionForecast[startTimeIndexInTimes + i * NoMeasurementsXHour] =
					forecastObj[i + startIndex].d;
			}
			...
			values.windSpeedForecast = calcInterpolation(
				windSpeedForecast,
				times,
				startTimeIndexInTimes,
				stopTimeIndexInTimes,
			);
			values.windGustsForecast = calcInterpolation(
				windGustsForecast,
				times,
				startTimeIndexInTimes,
				stopTimeIndexInTimes,
			);
			values.windDirectionForecast = calcInterpolation(
	...
	if (
		values.windSpeed.length === 0 &&
		values.windGusts.length === 0 &&
		values.windDirection.length === 0 &&
		values.windSpeedForecast &&
		isToday(dateParsed)
	) {
	...
	if (
		...
		log(
			`Location "${location.name}" doesn't have any data (neither measurements nor forecast)!`,
			"fetchError",
			true,
		);
