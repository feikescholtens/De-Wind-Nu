...
export async function fetchDataForDay_MVB(dateParsed, databaseData, resolve, times, DSTDates) {
  ...
  if (Object.keys(global.MVBAPIKey).length == 0 || (getUnixTime(new Date()) + 5) > global.MVBAPIKey.expirationDate) {
  ...
  async function fetchDataMVB(newToken) {
    ...
    rawData.Values.forEach(measurementType => {
      ...
      if (measurementType.ID.includes("WC3")) wind_gusts = tempArray.customCopy().map(x => x * 1.94384449)
      if (measurementType.ID.includes("WVC")) wind_speed = tempArray.customCopy().map(x => x * 1.94384449)
      if (measurementType.ID.includes("WRS")) wind_direction = tempArray.customCopy()
    })
