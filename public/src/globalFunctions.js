// Functions that are used globally accross multiple pages
...
export function setGeneralSettings() {
	//Sets the correct general settings under the "Instellingen" heading
	...
	if (localStorage.getItem("showBar") === "1") showBarSelector.checked = true;
		...
		if (document.querySelector("[data-dataForm]"))
...
export function updateLocalVariables() {
	...
	if (!localStorage.getItem("popupClickOnLocationSuggestionShowed"))
		localStorage.setItem("popupClickOnLocationSuggestionShowed", "0");
	//General settings
	...
	if (!localStorage.getItem("locationPreference"))
		localStorage.setItem("locationPreference", "low");
	if (!localStorage.getItem("userChoseLocationPreference"))
		localStorage.setItem("userChoseLocationPreference", "0");
...
export function changeTheme(newValue) {
	...
	if (
		location.pathname.substring(1, 5) === "wind" &&
...
export function changeShowBar(showBarSelector) {
	let value;
	const tabs = [
		document.querySelector("[data-overviewForm]"),
		document.querySelector("[data-dataForm]"),
	].filter((x) => x !== null)[0];
...
export function addUIListeners() {
	document
		...
		.addEventListener("click", () => displayPopUpWithName("over"));
	document
		.querySelector("[data-disclaimer]")
		.addEventListener("click", () => displayPopUpWithName("disclaimer"));
	document.querySelector("[data-feedback]").addEventListener("click", () => displayPopUpFeedback());
	document
		.querySelector("[data-credit]")
		.addEventListener("click", () => displayPopUpWithName("credit"));
	document
		.querySelector("[data-contact]")
		.addEventListener("click", () => displayPopUpWithName("contact"));
}
